...
import { IconTrofeu, IconPdf, IconX, IconSpinner } from "@/lib/icons";
  ...
  return (
    <div className="flex flex-wrap items-center gap-3">
      <button data-som="off" disabled={pend} onClick={() => start(async () => { const r = await publicar(provaId); setUrl(r.pdfUrl ?? url); setPub(true); somPublicar(); })}
        ...
        {pend ? <IconSpinner width={18} height={18} /> : <IconTrofeu width={18} height={18} />} {pend ? "Publicando…" : pub ? "Republicar resultados" : "Publicar resultados"}
      </button>
