...
export function FormPerfil({ valores }: { valores: { nome: string; postoGraduacao: string; telefone: string } }) {
  ...
  return (
    <form action={acao} className="flex flex-col gap-4 rounded-2xl border border-line bg-surf p-5 shadow-sm">
      <div className="grid grid-cols-[110px_1fr] gap-3">
        ...
        <div><label className={rotulo}>Nome <span className="text-red">*</span></label><input name="nome" defaultValue={valores.nome} required className={campoCad} /></div>
      </div>
